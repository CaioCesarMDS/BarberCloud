...
export class ServicesService {
    ...
    async createService(data: ServicesRequestDto): Promise<ServicesResponseDto> {
        try {
            const barbershop: Barbershop | null = await this.barbershopRepository.findById(data.barbershopId);
            if (barbershop) {
                const service: Services | null = await this.servicesRepository.create(data);
                if (service) {
    ...
    async getServiceById(id: number): Promise<ServicesResponseDto> {
        try {
            const service: Services | null = await this.servicesRepository.findById(id);
            if (service) {
                return new ServicesResponseDto(service);
            } else {
    ...
    async updateService(id: number, data: ServicesUpdateDto): Promise<ServicesResponseDto> {
        try {
            const service: Services | null = await this.servicesRepository.findById(id);
            if (service) {
                return new ServicesResponseDto(await this.servicesRepository.update(id, data));
            } else {
                throw new BadRequestException('Service Id is invalid!');
            }
        } catch (error) {
    ...
    async findAllFromBarbershopByName(barbershopId: string, name: string): Promise<ServicesResponseDto[]> {
        try {
            const barbershop: Barbershop | null = await this.barbershopRepository.findById(barbershopId);
            if (barbershop) {
                const services: Services[] | null = await this.servicesRepository.findAllFromBarbershopByName(barbershopId, name);
                if (services) {
    ...
    async getAllByBarbershopId(barbershopId: string): Promise<ServicesResponseDto[]> {
        try {
            const barbershop: Barbershop | null = await this.barbershopRepository.findById(barbershopId);
            if (barbershop) {
                const services: Services[] | null = await this.servicesRepository.getAllByBarbershop(barbershopId);
                if (services) {
                    return services.map(s => new ServicesResponseDto(s));
                } else {
                    throw new BadRequestException('Not found!');
