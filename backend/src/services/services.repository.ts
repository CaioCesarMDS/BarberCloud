...
export class ServicesRepository implements IServicesRepositoryInterface {
	...
	async findServiceMostPopularByClientId(clientId: string): Promise<ServiceWithTotal | null> {
	...
	async findServiceMostPopularByEmployeeId(employeeId: string): Promise<ServiceWithTotal | null> {
		const result = await this.prismaService.$queryRaw<ServiceWithTotal[]>`
			SELECT s.*, COUNT(*) AS "totalServices"
			FROM "ServicesOnScheduling" sos
			JOIN "Services" s ON s.id = sos."serviceId"
	...
	async findServiceMostPopularByBarbershopId(barbershopId: string): Promise<ServiceWithTotal | null> {
		const result = await this.prismaService.$queryRaw<ServiceWithTotal[]>`
			SELECT s.*, COUNT(*) AS "totalServices"
			FROM "ServicesOnScheduling" sos
			JOIN "Services" s ON s.id = sos."serviceId"
